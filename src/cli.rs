...
pub(crate) async fn run(pusb: USB, pa11: PA11, mut pa12: PA12) {
    {
    ...
    let usb_fut = usb.run();
    let echo_fut = transfer(&mut class);

    join(usb_fut, echo_fut).await;
}

async fn transfer<'d, T: Instance + 'd>(class: &mut CdcAcmClass<'d, Driver<'d, T>>) {
    loop {
